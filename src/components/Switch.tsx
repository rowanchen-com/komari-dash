...
export default function Switch({ allTag, nowTag, tagCountMap, onTagChange }: SwitchProps) {
  const { info } = usePublicInfo()
  const { t, locale } = useLocale()
  const showTagCount = getThemeSetting(info?.theme_settings, "showTagCount", false)
  ...
  useEffect(() => {
    ...
    if (currentTagElement) {
      setIndicator({ x: currentTagElement.offsetLeft, w: currentTagElement.offsetWidth })
    }
    if (isFirstRender) {
          ...
          >
            <div className="relative z-20 flex items-center gap-1">
              <div className="flex items-center gap-2 whitespace-nowrap">
                {tag === "defaultTag" ? t("ServerList", "defaultTag") : tag}
                {showTagCount && tag !== "defaultTag" && tagCountMap[tag] && (
